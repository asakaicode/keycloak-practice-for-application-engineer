...
- [x] Part 1: 15分で動かす（Docker 起動 → ログイン成功体験）
- [x] Part 2: 概念編（IdP の存在理由 / OAuth2・OIDC メンタルモデル / 認可コードフロー+PKCE のシーケンス図）
- [x] Part 3: ハンズオン本編 3.1〜3.3（Realm/Client/User 作成、keycloak-js で SPA 保護、トークン深掘り）
- [x] 各ページに「今すぐ試す」ボックス（コピー可能なコマンド + 期待される結果）を必ず1つ置く
- [x] 自分自身で全ハンズオンを最初から通しで実施して検証（執筆者チェック = 最初の読者テスト）
...
- [x] Part 3 残り（ログアウト/セッション、ロール・クレームの UI 反映、API 保護と CORS）
- [x] Part 4: 現実と向き合う（サードパーティ Cookie 問題、BFF パターン、dev vs prod）
- [x] Part 5: エラー体験ラボ（redirect_uri 不一致 / CORS / トークン期限切れを意図的に起こして直す）
- [x] Part 6: リファレンス（用語対訳表、公式ドキュメントの歩き方、エンドポイント早見表）
- [x] 仕掛けの実装: ページ末尾チェックリスト（localStorage）、章末クイズ、JWT デコーダウィジェット
- [x] 全ページ再通読 + ハンズオン再検証
